...
void RenderInterfaceDirectX10::setupEffect()
{
	...
	if (FAILED(D3DX10CreateEffectFromMemory((void*)pEffectData,strlen(pEffectData),"DefaultEffect",NULL,NULL,"fx_4_0",dwShaderFlags,0,m_pD3D10Device,NULL,NULL,&m_pEffect,&errorBuffer,NULL)))
	{
    ...
    m_pD3D10Device->CreateRasterizerState(&rasterizerDesc, &rs_scissorsOff);
 
    rasterizerDesc.ScissorEnable = true;
    m_pD3D10Device->CreateRasterizerState(&rasterizerDesc, &rs_scissorsOn);

	m_pD3D10Device->RSSetState(rs_scissorsOn);
}
...
RenderInterfaceDirectX10::~RenderInterfaceDirectX10()
{
	if (rs_scissorsOff)
	{
		rs_scissorsOff->Release();
	}
	if (rs_scissorsOn)
	{
		rs_scissorsOn->Release();
	}
}
...
void RenderInterfaceDirectX10::RenderCompiledGeometry(Rocket::Core::CompiledGeometryHandle geometry, const Rocket::Core::Vector2f& translation)
{
	...
	{ 
		m_pD3D10Device->RSSetState(rs_scissorsOff);
	}
